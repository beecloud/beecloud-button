...
instant\_channel | String | 设置该字段后将直接调用渠道支付，不再显示渠道选择菜单 | "ali"(ALI_WEB、ALI_WAP), "wxmp"(WX_NATIVE扫码), "bcwxmp"(BC_NATIVE扫码), "wx"(WX_JSAPI微信网页内支付), bcwx(BC_WX_JSAPI), un(UN_WEB), unwap(UN_WAP), bckj(BC_EXPRESS), bcwxwap(BC_WX_WAP微信移动支付) "un"中的一个 | "ali" | 否
